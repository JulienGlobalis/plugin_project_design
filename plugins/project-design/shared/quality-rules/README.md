...
Keep source intake separate from delivery. After the documentary choice and
before source collection, ask whether the user wants to keep sources at their
original locations or centralize them under `_sources/` at the target project
root. Never create or reuse `_sources/` without explicit confirmation.

The centralized workspace has this stable structure:
...
`source-index.md` and `links.md` are source-management metadata, not business
or documentary deliverables, and are therefore the only plugin-generated
Markdown exceptions to the `_project-design/` delivery rule. Add
`/_sources/` to the target project's `.gitignore` by default. Copy a local
file only after explicit per-file approval, preserve its original, never
overwrite an existing destination, and record its original path and checksum.
Record Google Drive-native sources as links without exporting them unless the
user separately requests an export. The original source remains authoritative.
