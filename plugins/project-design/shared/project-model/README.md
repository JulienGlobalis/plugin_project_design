...
- "Reduce lost requests" can become an `Objective` element linked to a
  `Need`, rather than remaining a sentence copied from the incomplete-project
  context note.
- The proposed calendar synchronization in the new-application roadmap can
  become an `Option` element related to an `Integration` element; it must not
  become established target Scope merely because it was mentioned.
...
- The relationship between department membership and an approving manager is
  unresolved because the technical assumptions state that the identity team
...
- The relationships from the nightly directory `Integration` element to its
  ...
  provisional, depending on review of the interface notes.
- The target approval `Business Rule` governs the place-request `Process` in
  the new-application service rules.
...
- The shared mailbox and spreadsheet workflow in the incomplete-project
  workshop has Existing perspective, while request submission through a
  future workspace has Target perspective.
...
- Migration sequencing, rollback, training, and parallel operation in the
  modernization constraints have Transition perspective.
