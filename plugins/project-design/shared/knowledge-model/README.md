...
Examples from the permanent development corpus:

- The Member Service Portal charter states that closed requests and
  attachments must remain searchable for five years.
- The service policy extract states a seven-year retention period.
- The incomplete-project technical intake does not supply data retention
  requirements. This may support a
...
- "Single sign-on is preferred" in the incomplete-project technical intake
  can reference `Integration` and `Assumption`.
- The proposed calendar synchronization in the new-application roadmap can
  reference `Integration` and `Option`.
- The unapproved migration sequencing in the modernization constraints can
  reference `Transition`, `Option`, and `Open Question`.
...
- The 2019 user-guide extract is explicitly dated and approved, but no later
  revision was supplied.
- The support observations report more recent operational behavior without
  establishing formal authority.
- The new-application integration notes identify several unknown interface
  details and the teams expected to confirm them.
...
- The incomplete-project vocabulary notes make ambiguity about `request`,
  `ticket`, and `case` explicit.
- The new-application roadmap identifies future ideas as proposals rather
  than approved commitments.
- The modernization decision history records decisions while also stating
  that their current applicability is unknown.
...
- The five-year retention assertion in the project charter and seven-year
  assertion in the service policy can be related as `Contradicts` for the
  same records and period of applicability.
- The 2019 statement that closed cases cannot be reopened and the user
  workshop statement that advisors reopen cases require a contradiction
  relationship; recency alone does not establish supersession.
- The new-application business objective to make approvals visible is
  supported and refined by the manager interview.
